...
GOOD = (
    '{"resolved": true, "score": 0.9, "reasoning": "all three ran", "failed_steps": []}'
)
...
def test_judge_spend_stays_out_of_mission_spend(isolated_ledger):
    ...
    ledger.record_call("m", "sonnet", {"input_tokens": 100, "output_tokens": 10}, 1.00)
...
def test_a_long_transcript_keeps_the_ending():
    ...
    body = "\n".join(
        f"[AIMessage] STEP {i} DONE: fine and padded out" for i in range(1, 1500)
    )
