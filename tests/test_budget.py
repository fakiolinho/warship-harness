...
def test_crossing_the_ceiling_pauses_the_mission(isolated_ledger):
    ...
    with pytest.raises(MissionPaused):
        g.after_model(
            {"messages": [_Msg({"input_tokens": 1_000_000, "output_tokens": 0})]}
        )
...
def test_staying_under_the_ceiling_does_not_pause(isolated_ledger):
    ...
    assert (
        g.after_model({"messages": [_Msg({"input_tokens": 10, "output_tokens": 1})]})
        is None
    )
...
def test_the_ceiling_refuses_before_dispatch():
    ...
    with pytest.raises(MissionPaused, match="refused before dispatch"):
        g.wrap_model_call(
            _Req([HumanMessage(content="x" * 400_000)]), lambda r: dispatched.append(r)
        )
    assert dispatched == []
...
def test_an_affordable_call_is_dispatched():
    ...
    assert (
        g.wrap_model_call(_Req([HumanMessage(content="hi")]), lambda r: "ran") == "ran"
    )
...
def test_the_floor_follows_max_output_tokens():
    assert (
        BudgetGuard(max_output_tokens=1_000).single_call_floor()
