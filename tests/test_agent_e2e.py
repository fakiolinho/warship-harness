...
def _run(mission, model, ceiling=5.00, approver=lambda n, a: False):
    ...
    agent, gate = agent_module.build_agent(
        mission, budget, model=model, approver=approver, compaction_model=None
    )  # no summarizer: it would need a real model
    steps = []
    for event in agent.stream(
        {"messages": [("user", state.resume_prompt(mission))]}, stream_mode="values"
    ):
...
def test_a_clean_run_checkpoints_every_step(mission, isolated_ledger):
    model = ScriptedModel(
        script=[
            ...
            ai(
                "STEP 2 DONE: second",
                [call("run_command", {"command": "echo b"}, "c2")],
            ),
...
def test_oversized_tool_output_is_persisted_not_inlined(mission, isolated_ledger):
    model = ScriptedModel(
        script=[
            ai(
                "reading",
                [call("run_command", {"command": "python3 -c \"print('x'*20000)\""})],
            ),
...
def test_the_budget_ceiling_stops_a_runaway(mission, isolated_ledger):
    ...
    model = ScriptedModel(
        script=[ai(f"STEP {i} DONE: step {i}") for i in range(1, 11)], calls=[]
    )
    try:
...
def test_a_setup_failure_explains_itself(mission, isolated_ledger, monkeypatch, capsys):
    monkeypatch.chdir(mission)
...
def test_a_failure_after_the_mission_started_is_not_swallowed(
    ...
    model = _FlakyModel(
        script=[ai("STEP 1 DONE: one", [call("run_command", {"command": "echo hi"})])],
        calls=[],
    )
