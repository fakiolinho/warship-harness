...
def seed(out: pathlib.Path, n: int, rng: random.Random,
         judge_fraction: float = 0.5) -> dict:
    ...
    for i in range(n):
        ...
        for c in range(calls):
            ...
            rows.append({
                "kind": "call", "mission": mission, "model": "sonnet",
        ...
        if rng.random() < judge_fraction:
            ...
            rows.append({
                "kind": "judge", "mission": mission, "model": "sonnet",
                "rubric": "v1", "resolved": resolved, "score": round(score, 3),
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("-n", "--missions", type=int, default=40,
                   help="how many missions to fabricate (default: 40)")
    p.add_argument("--seed", type=int, default=1729,
                   help="RNG seed; same seed gives the same file")
    p.add_argument("--judge-fraction", type=float, default=0.5,
                   help="share of missions graded by a judge (default: 0.5)")
    args = p.parse_args(argv)

    if args.out.name == ledger.DEFAULT_LEDGER or args.out == ledger.path():
        sys.exit(f"error: refusing to write {args.out}, which is a real "
                 "ledger. Fabricated rows are indistinguishable from real "
                 "ones once they are in there. Pick another path.")

    stats = seed(args.out, args.missions, random.Random(args.seed),
                 args.judge_fraction)
    print(f"wrote {stats['rows']} rows for {args.missions} missions "
          f"to {args.out}")
    print(f"  {stats['resolved']} resolved, {stats['judged']} judged, "
