...
# ponytail: a fixed line. Upgrade path: a percentile split once the
# ledger has enough volume for percentiles to mean anything.
HOT_SPEND_USD = 1.00
...
def analyze(rows: list[dict]) -> dict:
    ...
    return {
        ...
        "cost_per_resolved_task": total_cost / len(resolved)
        if resolved
        else float("inf"),
...
def injection_note(a: dict) -> str:
    ...
    if not hit:
        return ""
    return (
        f"> **The judge reported an instruction attempt in the "
...
def synthetic_note(a: dict) -> str:
    ...
    real = total - fake
    return (
        f"> **This ledger mixes {fake} fabricated "
...
def cache_note(a: dict) -> str:
    ...
    if a.get("cache_written_tokens", 0) == 0:
        return (
            "> **Cache hit rate is 0% because caching is not enabled.** "
    ...
    return (
        "> **Cache hit rate is 0% but tokens were written to cache.** "
...
def render(a: dict) -> str:
    lines = [
        ...
        f"| interventions per mission | {a['interventions_per_mission']:.1f} |",
        f"| total spend | ${a['total_cost']:.2f} |",
        "",
    ]
    if not a["missions"]:
        lines.append(
            f"No missions in the ledger yet ({ledger.path()}). "
    ...
    for note in (injection_note(a), synthetic_note(a), cache_note(a)):
        if note:
            lines += [note, ""]
    lines += [
        "| mission | spend | calls | steps | resolved | by | quadrant |",
        "|---|---|---|---|---|---|---|",
    ]
    ...
    if a["judged_missions"]:
        ...
        lines += [
            "",
            "> Every verdict above is the step count proxy: the "
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(
        prog="finops.py", description="Task economics report from the ledger."
    )
    p.add_argument(
        "-o",
        "--output",
